...
<script type="text/javascript">
	function pNum(pid,p){
		$.ajax({
            url:"cartController?method=updateCartNum&pid="+pid+"&flag=1&price="+p,
            method:"get",
            success:function(){
	...
	function clearCart(pid){
		if(confirm("确认要删除吗")){
            $.ajax({
                //delNum
                url:"cartController?method=delCart&pid="+pid,
                method:"get",
                success:function(data){
                    if (data.code==1){
                        location.href = "cartController?method=getCart";
						...
						<!--private  Product product;//一条购物车信息对应一个商品-->
	 					...
	 					<th width="100px">
		 					<div class="input-group">
		 						...
		 						<span class="input-group-btn">
		 							<button class="btn btn-default" type="button" onclick="pNum(${c.product.pid},${c.product.pprice})">+</button>
		 						</span>
	 						</div>
	 					</th>
	 					<th>¥&nbsp;${c.ccount }</th>
